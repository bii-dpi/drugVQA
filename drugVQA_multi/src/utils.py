...
def create_variable(tensor, gpu):
    # Do cuda() before wrapping with variable
    if torch.cuda.is_available():
        return Variable(tensor.cuda(gpu))
    else:
...
def make_variables(lines, properties, letters, gpu):
    sequence_and_length = [line2voc_arr(line, letters) for line in lines]
    vectorized_seqs = [sl[0] for sl in sequence_and_length]
    seq_lengths = torch.LongTensor([sl[1] for sl in sequence_and_length])
    return pad_sequences(vectorized_seqs, seq_lengths, properties, gpu)


def make_variables_seq(lines, letters, gpu):
    sequence_and_length = [line2voc_arr(line, letters) for line in lines]
    vectorized_seqs = [sl[0] for sl in sequence_and_length]
    seq_lengths = torch.LongTensor([sl[1] for sl in sequence_and_length])
    return pad_sequences_seq(vectorized_seqs, seq_lengths, gpu)
...
def pad_sequences(vectorized_seqs, seq_lengths, properties, gpu):
    ...
    return create_variable(seq_tensor, gpu),create_variable(seq_lengths, gpu), create_variable(target, gpu)


def pad_sequences_seq(vectorized_seqs, seq_lengths, gpu):
    seq_tensor = torch.zeros((len(vectorized_seqs), seq_lengths.max())).long()
    for idx, (seq, seq_len) in enumerate(zip(vectorized_seqs, seq_lengths)):
        seq_tensor[idx, :seq_len] = torch.LongTensor(seq)
...
#     print(seq_tensor)
    ...
    # Return variables
    # DataParallel requires everything to be a Variable
    return create_variable(seq_tensor, gpu), create_variable(seq_lengths, gpu)
