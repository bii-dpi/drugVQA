...
class DrugVQA(nn.Module):
    ...
    def __init__(self, args, block):
        ...
        self.lstm = torch.nn.LSTM(args['emb_dim'], self.lstm_hid_dim, 2,
        ...
        self.hidden_state = self.init_hidden(args['gpu'])
        self.seq_hidden_state = self.init_hidden(args['gpu'])
    ...
    def init_hidden(self, gpu):
        return (Variable(torch.zeros(4, self.batch_size, self.lstm_hid_dim).cuda(gpu)),
                Variable(torch.zeros(4, self.batch_size, self.lstm_hid_dim)).cuda(gpu))
    ...
    def forward(self, x1, x2):
        ...
        #print("\n\n\n\n\n")
        #print("initial", x2.shape)
        pic = self.conv(x2)
        #print("after first conv", pic.shape)
        pic = self.bn(pic)
        #print("after bn", pic.shape)
        pic = self.elu(pic)
        #print("after elu", pic.shape)
        pic = self.layer1(pic)
        #print("after first block", pic.shape)
        pic = self.layer2(pic)
        #print("after second block", pic.shape)
        pic_emb = torch.mean(pic, 2)
        #print("after getting embedding", pic_emb.shape)
        pic_emb = pic_emb.permute(0, 2, 1)
        #print("after scrambling embedding", pic_emb.shape)
        seq_att = F.tanh(self.linear_first_seq(pic_emb))       
        #print("after first linear layer", seq_att.shape)
        seq_att = self.linear_second_seq(seq_att)       
        #print("after second linear layer", seq_att.shape)
        seq_att = self.softmax(seq_att, 1)       
        #print("after softmax", seq_att.shape)
        seq_att = seq_att.transpose(1, 2)
        #print("after transpose", seq_att.shape)
        seq_embed = seq_att@pic_emb      
        #print("seq embed", seq_embed.shape)
        avg_seq_embed = torch.sum(seq_embed, 1)/self.r
        #print("avg seq embed", avg_seq_embed.shape)
