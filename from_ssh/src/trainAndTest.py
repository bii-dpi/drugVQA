...
def train(trainArgs):
    ...
    curr_path = f"../model_pkl/DUDE/{trainArgs['saveNamePre']}{30}.pkl"
    trainArgs['model'] = DrugVQA(modelArgs,block = ResidualBlock).cuda()
    trainArgs['model'].load_state_dict(torch.load(curr_path))
    ...
    for i in progressbar(range(trainArgs['epochs'])):
        ...
        if (trainArgs['doSave']):
            torch.save(attention_model.state_dict(),
                        f"../model_pkl/DUDE/{trainArgs['saveNamePre']}{i + 31}.pkl")
    ...
    # done
    with open(f"../results/{trainArgs['saveNamePre']}train_results_2.csv", "w") as f:
        f.write(csv)
...
def test(testArgs, epoch):
    ...
    with open(f"../results/{testArgs['saveNamePre']}test_results_2.csv", "a") as f:
        f.write(csv)
