...
def get_examples(target, interactivity_type):
    ...
    with open(f"data/DUDE/{interactivity_type}_smile/"
...
def write_shuffled_data(example_dict, mode):
    """Get shuffled list of active and decoy examples. """
    examples = example_dict[mode]["actives"] + example_dict[mode]["decoys"]
    np.random.shuffle(examples)
    with open(f"data/DUDE/dataPre/{mode}_fold", "w") as f:
        f.write("\n".join(examples))


# Application
# Load all 49 target names.
all_targets = \
